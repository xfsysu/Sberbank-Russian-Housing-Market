...
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=420)
...
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=422)
y_predict = model.predict(dtest)
...
train = pd.read_csv('train.csv', parse_dates=['timestamp'])
test = pd.read_csv('test.csv', parse_dates=['timestamp'])
name_to_subarea = pd.read_csv('NAME_to_subarea.csv')
id_test = test.id
...
# drop feature
train = train.drop(['culture_objects_top_25_raion', 'oil_chemistry_raion', 'railroad_terminal_raion', 'nuclear_reactor_raion', 'build_count_foam', 'big_road1_1line', 'railroad_1line', 'office_sqm_500', 'trc_sqm_500', 'cafe_count_500_price_4000', 'cafe_count_500_price_high', 'mosque_count_500', 'leisure_count_500', 'office_sqm_1000', 'trc_sqm_1000', 'cafe_count_1000_price_high', 'mosque_count_1000', 'cafe_count_1500_price_high', 'mosque_count_1500', 'cafe_count_2000_price_high'], axis=1)
test = test.drop(['culture_objects_top_25_raion', 'oil_chemistry_raion', 'railroad_terminal_raion', 'nuclear_reactor_raion', 'build_count_foam', 'big_road1_1line', 'railroad_1line', 'office_sqm_500', 'trc_sqm_500', 'cafe_count_500_price_4000', 'cafe_count_500_price_high', 'mosque_count_500', 'leisure_count_500', 'office_sqm_1000', 'trc_sqm_1000', 'cafe_count_1000_price_high', 'mosque_count_1000', 'cafe_count_1500_price_high', 'mosque_count_1500', 'cafe_count_2000_price_high'], axis=1)

bad_index = train[train.build_year > 2017].index
train.ix[bad_index, "build_year"] = np.NaN
bad_index = test[test.build_year > 2016].index
test.ix[bad_index, "build_year"] = np.NaN


# add 6.24 Missing value fill
...
y_predict = model.predict(dtest)
output = pd.DataFrame({'id': id_test, 'price_doc': y_predict})
output.to_csv('xgbSub0.csv', index=False)


#################################################################################################################
################################################## final result #################################################
sub = pd.read_csv('mix.csv')
sub['price_doc'] = sub['price_doc'] * 0.65 + output.price_doc * 0.35
